...
function getRandomAmount(): string {
    const amounts = ["1000", "2500", "5000", "10000", "15000", "20000", "Full Tuition"];
    ...
    for (const uni of universities) {
        ...
        for (let i = 0; i < numScholarships; i++) {
            ...
            if (programs.length > 0) {
                ...
                const numProgramsToConnect = Math.min(programs.length, Math.floor(Math.random() * 3) + 1);
                const shuffled = [...programs].sort(() => 0.5 - Math.random());
                connectedPrograms = shuffled.slice(0, numProgramsToConnect).map(p => ({ id: p.id }));
            }
