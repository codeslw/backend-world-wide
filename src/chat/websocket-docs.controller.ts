...
export class WebSocketDocsController {
  ...
  @WebSocketDoc({ 
    ...
    description: 'Connect to the WebSocket server using Socket.IO client with JWT token for authentication.'
  })
  @ApiBody({ 
  ...
  @WebSocketDoc({ 
    ...
    description: 'Join a specific chat room to receive real-time messages and updates. Returns recent messages from the chat.'
  })
  @ApiBody({ 
  ...
  @ApiResponse({ status: 403, description: 'Access denied', type: WebSocketErrorDto })
  @ApiResponse({ status: 404, description: 'Chat not found', type: WebSocketErrorDto })
  joinChat() {
  ...
  @WebSocketDoc({ 
    ...
    description: 'Leave a specific chat room to stop receiving real-time updates for that chat.' 
  })
  @ApiBody({ 
    description: 'Chat ID (string)', 
    schema: {
  ...
  @WebSocketDoc({ 
    ...
    description: 'Send a text message, file attachment, or reply to another message within a chat.' 
  })
  ...
  @WebSocketDoc({ 
    ...
    description: 'Indicate when a user is typing or has stopped typing in a chat.' 
  })
  ...
  @WebSocketDoc({ 
    ...
    description: 'Mark one or more messages as read by the current user. Updates read receipts for all chat participants.' 
  })
  ...
  @WebSocketDoc({ 
    ...
    description: 'Retrieve a list of users currently active in a specific chat room.' 
  })
  @ApiBody({ 
    description: 'Chat ID (string)', 
    schema: {
  ...
  @WebSocketDoc({ 
    ...
    description: 'Received when a new message is sent to a chat the client has joined.' 
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @WebSocketDoc({ 
    ...
    description: 'Received when the status of a chat changes (active, pending, closed).' 
  })
  @ApiResponse({ 
    status: 200, 
