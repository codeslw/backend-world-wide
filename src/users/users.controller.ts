...
import { ApiTags, ApiOperation, ApiParam, ApiBearerAuth, ApiQuery, ApiResponse, ApiBody } from '@nestjs/swagger';
...
export class UsersController {
  ...
  //it shows ['string'] for payload on swagger what is the problem?
  @ApiBody({ type: [CreateUserDto] })
  @Post('create/many')
  @ApiOperation({ summary: 'Register multiple users' })
  @ApiResponse({ status: 201, description: 'Users successfully created', type: [UserResponseDto] })
  @ApiResponse({ status: 400, description: 'Bad request - Invalid data provided', type: ErrorResponseDto })
  @ApiResponse({ status: 409, description: 'Email already in use', type: ErrorResponseDto })
  ...
  @ApiQuery({ name: 'sortDirection', description: 'Sort direction', required: false })
