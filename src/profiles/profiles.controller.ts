...
export class ProfilesController {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required', type: ErrorResponseDto })
  @ApiResponse({ status: 404, description: 'Profile not found', type: ErrorResponseDto })
  ...
  async update(
    ...
    if (profile.userId !== req.user.userId && req.user.role !== Role.ADMIN) {
      throw new ForbiddenActionException('You do not have permission to update this profile');
    }
  ...
  @ApiParam({ name: 'id', description: 'Profile ID' })
  @ApiResponse({ status: 200, description: 'Profile deleted successfully', type: ProfileResponseDto })
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required', type: ErrorResponseDto })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions', type: ErrorResponseDto })
  @ApiResponse({ status: 404, description: 'Profile not found', type: ErrorResponseDto })
