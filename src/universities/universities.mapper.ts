...
import { University, Country, City, Program, UniversityProgram, Intake, UniversityRequirements, Scholarship } from '@prisma/client';
...
export class UniversitiesMapper {
    toResponseDto(university: UniversityWithRelations, lang: string = 'uz'): UniversityResponseDto {
        ...
        return {
            ...
            scholarships: university.scholarships?.map(s => ({
                ...
                renewalConditions: s.renewalConditions as any,
                eligibility: s.eligibility as any,
    ...
    toListItemDto(university: UniversityWithRelations, lang: string = 'uz'): UniversityListItemDto {
        const langSuffix = this.getLangSuffix(lang);
        const countryName = this.getLocalizedField(university.country, 'name', langSuffix);
        const cityName = this.getLocalizedField(university.city, 'name', langSuffix);
    ...
    toMainUniversityDto(university: UniversityWithRelations, lang: string = 'uz'): MainUniversityResponseDto {
        const langSuffix = this.getLangSuffix(lang);
    ...
    toUniversityByProgramDto(up: UniversityProgramWithRelations, lang: string = 'uz'): UniversityByProgramResponseDto {
        const langSuffix = this.getLangSuffix(lang);
    ...
    toProgramDetailsDto(up: UniversityProgram & { program: Program; scholarships?: Scholarship[] }, lang: string = 'uz'): ProgramDetailsDto {
        const langSuffix = this.getLangSuffix(lang);
        const program = up.program;
        ...
        return {
            ...
            studyLevel: up.studyLevel as StudyLevel,
            duration: up.duration,
            scholarships: up.scholarships?.map(s => ({
                id: s.id,
                title: s.title,
                description: s.description || '',
                institutionName: s.institutionName,
    ...
    private getLocalizedField(entity: any, fieldPrefix: string, langSuffix: string): string {
        if (!entity) return '';
        return entity[`${fieldPrefix}${langSuffix}`] || entity[`${fieldPrefix}Uz`] || '';
    }
